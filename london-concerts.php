...
Plugin Name: London Music Concerts
Version: 1.0
Plugin URI: http://www.londondrum.com/events/?page_id=5270
Description: Displays a list of upcoming rock/pop concerts in London. All of the big venues are included: O2 Arena, Wembley Arena, Roundhouse, HMV Forum, Royal Albert Hall... and more.
Author: London Drum
...
function london_music_concerts() {
	...
	echo "<div id=\"london_events\"><script src=\"http://feeds.feedburner.com/LondonMusicConcerts?format=sigpro&nItems=".$itemnum."&displayExcerpts=".$itemdescr."&displayDate=".$hidedate."&dateFormat=".$itemdate."\" type=\"text/javascript\"></script><br style=\"clear:left;\"><sub><a href=\"http://www.londondrum.com/events/?cat=856\">Music gigs in London</a> &middot; <a href=\"http://www.londondrum.com/frm/\">London forum</a></sub></div>";
...
function modify_menu() { // creates the admin page
	...
	add_options_page(
	'London Music Concerts', 'London Music Concerts', 'manage_options', _File_, 'admin_londonevents_options'
	);
...
function admin_londonevents_options() { // creates the admin settings
	...
	<h2 style=\"margin:3px -11px 8px;padding:3px 6px;background:#999;color:#FAFAFA;font:bold 22px/22px arial,verdana,sans-serif;text-transform:uppercase;\">London Music Concerts</h2>");
...
#london_events li {   // containing block for each concert
	display: block; clear: both; padding-bottom: 2px;
	}
#london_events li .headline {   // link to the concert
	display: block; clear: both; font-size: 1.2em;
	}
#london_events li .date {   // date of the concert
	color: #666; margin: 0;
	}
#london_events li div p img {   // image of the band/artist
...
#london_events sub {   // credit
	...
	echo "<p>There is a preview of the feed at the bottom of the page.</p>";
		...
		echo "<h3 style=\"color:#444;font:bold 16px/16px arial,verdana,sans-serif;text-transform:uppercase;\">Step 1: &nbsp;<span style=\"color:#777;text-transform:none;\">Choose the number of concerts</span></h3>
	
	<p>You can show between 1 and 10 concerts:</p>";
	...
	(This will obviously have no effect if you decide to hide the date.)</p>";
		...
		echo "<h3 style=\"color:#444;font:bold 16px/16px arial,verdana,sans-serif;text-transform:uppercase;\">Step 6: &nbsp;<span style=\"color:#777;text-transform:none;\">Add the concerts to your site</span></h3>
		<p>There are two ways of adding the concerts to your web page.<br>
		1) You can navigate to 'Appearance > Widgets' and drag the 'London Music Concerts' widget onto your sidebar<br>
		2) You can add the following line to one of your Wordpress templates, where you&#8217;d like the concerts to appear:</p>
		<div style=\"width:70%;padding:1em;background:#FFF;border:1px solid #AAA;\"><code>&lt;?php london_music_concerts(); ?&gt;</code></div>";
		...
		echo "<script src=\"http://feeds.feedburner.com/LondonMusicConcerts?format=sigpro&nItems=".$itemnum."&displayExcerpts=".$itemdescr."&displayDate=".$hidedate."&dateFormat=".$itemdate."\" type=\"text/javascript\"></script>";
...
{
  register_sidebar_widget(__('London Music Concerts'), 'widget_londonevents'); 
  register_widget_control(   'London Music Concerts', 'londonevents_control', 200, 200 );
}
