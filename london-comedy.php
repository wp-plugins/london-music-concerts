...
function london_comedy_gigs() {
	...
	echo "<div id=\"london_comedy\"><script src=\"http://feeds.feedburner.com/ComedyShowsInLondon?format=sigpro&nItems=".$itemnum."&displayExcerpts=".$itemdescr."&displayDate=".$hidedate."&dateFormat=".$itemdate."\" type=\"text/javascript\"></script><br style=\"clear:left;\"><sub><a href=\"http://www.londondrum.com/events/?cat=9\">Comedy in London</a></sub></div>";
...
function modify_menu() { // creates the admin page
	...
	add_options_page(
	'London Comedy Gigs', 'London Comedy Gigs', 'manage_options', _File_, 'admin_londonevents_options'
	);
...
function admin_londonevents_options() { // creates the admin settings
	...
	<h2 style=\"margin:3px -11px 8px;padding:3px 6px;background:#999;color:#FAFAFA;font:bold 22px/22px arial,verdana,sans-serif;text-transform:uppercase;\">London Comedy Gigs</h2>");
...
function print_londonevents_form() { // prints out the admin settings
	...
	$styles = get_option('event_styles');
		if($styles==''){$styles='#london_comedy {   // main containing block
	background: transparent; border: none;
	}
#london_comedy li {   // containing block for each gig
	display: block; clear: both; padding-bottom: 2px;
	}
#london_comedy li .headline {   // link to the gig
	display: block; clear: both; font-size: 1.2em;
	}
#london_comedy li .date {   // date of the gig
	color: #666; margin: 0;
	}
#london_comedy li div p img {   // image of the comedian
	display: inline; float: left; margin: 0 10px 6px 0 !important; border: 1px solid #CCC; vertical-align: top;
	}
#london_comedy sub {   // credit
	...
	<p>Want to know where the comedy gigs come from?</p>
	<p>The shows are kept fresh at <a href=\"http://www.londondrum.com/\">www.londondrum.com</a>, so you&#8217;d need never worry about updating them.</p>
	</div>";
	
	echo "<p>There is a preview of the feed at the bottom of the page.</p>";
	
		
		echo "<h3 style=\"color:#444;font:bold 16px/16px arial,verdana,sans-serif;text-transform:uppercase;\">Step 1: &nbsp;<span style=\"color:#777;text-transform:none;\">Choose the number of gigs</span></h3>
	...
	(This will obviously have no effect if you decide to hide the date.)</p>";
		...
		echo "<h3 style=\"color:#444;font:bold 16px/16px arial,verdana,sans-serif;text-transform:uppercase;\">Step 6: &nbsp;<span style=\"color:#777;text-transform:none;\">Add the gigs to your site</span></h3>
		<p>There are two ways of adding the shows to your web page.<br>
		1) You can navigate to 'Appearance > Widgets' and drag the 'London Comedy Gigs' widget onto your sidebar<br>
		2) You can add the following line to one of your Wordpress templates, where you&#8217;d like the shows to appear:</p>
		<div style=\"width:70%;padding:1em;background:#FFF;border:1px solid #AAA;\"><code>&lt;?php london_comedy_gigs(); ?&gt;</code></div>";
		...
		<div style=\"width:70%;margin-bottom:.1em;padding:1em 1em .5em;background:#FFF;border:1px solid #AAA;\"><div id=\"london_comedy\">";
		
		echo "<script src=\"http://feeds.feedburner.com/ComedyShowsInLondon?format=sigpro&nItems=".$itemnum."&displayExcerpts=".$itemdescr."&displayDate=".$hidedate."&dateFormat=".$itemdate."\" type=\"text/javascript\"></script>";
...
{
  register_sidebar_widget(__('London Comedy Gigs'), 'widget_londonevents'); 
  register_widget_control(   'London Comedy Gigs', 'londonevents_control', 200, 200 );
}
